...
The latest mobile Studio screenshot confirms that the selected-image state is reachable from Unorganised and the interface explains automatic Gemini checks. It also exposes a missing manual recovery/action path: the selected-image tray currently offers a custom-album selector, Organize images, permanent deletion, and Clear, but no explicit action to submit existing selected unorganised images to Gemini again.

The same screenshot shows a clear two-column image grid with visible per-image selection controls, practical grid/list/sort controls, and a selection tray that appears only after two images are selected. The gap is action coverage rather than discoverability: a direct manual Gemini submission should sit in that selected-image tray ahead of the album-management control, with a disabled/loading state that explains when automatic review is already pending.

The selected-image tray now exposes **Send 1 to AI** (or the corresponding selected count) before album management. It clearly states that it creates editable Gemini suggestions without moving an image. A live verification on image `1000744753` transitioned through **Gemini checking…** and returned an editable **Embossed Deity Bangles** review, suggesting the existing Bangles album while the image remained in Unorganised with zero automatic membership change.

For additional mobile speed, the gallery now persists a compact 560 px preview alongside the existing 960 px thumbnail and original. The bounded backfill returned `23 / 23` preview URLs ready. The Library’s compact cards now request preview URLs, with the original retained for full-image viewing and Immersive presentation.

Final validation for this refinement passed: 64 unit tests, TypeScript checking, and the production build all completed successfully. The selected-image action was verified live; it did not move the image or create an album, and its suggestion remains available for the user’s explicit review decision.

The final progressive-loading review confirmed compact preview delivery in the Studio Library, Home recent grid, public Albums covers, and Bangles album-detail grid. Every compact surface now uses the same subtle skeleton-to-image reveal with the 560 px preview derivative; the album-detail wrapper uses a semantic `picture` element so its caption styling cannot conceal the preview. A live visual check confirmed the Bangles previews displayed correctly after the adjustment, while the full-image viewer and Immersive playback still retain original sources. The completed regression run reports 64 tests, a clean TypeScript check, and a successful production build.

Final proof: the 390 px album-detail capture visibly shows all three Bangles preview tiles after the semantic wrapper correction. Focused `GalleryPreviewImage` tests cover the loading state and both successful/error reveal states. The complete final validation reports 66 tests passing, a clean TypeScript check, and a successful production build.
...
## Persistent Needs Review Queue Verification

Duplicate candidates now persist in `duplicateReviewCandidates` with their storage metadata, match target, match kind, fingerprint values, status, decision, and timestamps. Uploads return an accepted review-needed result after candidate media and derivatives are stored; they do not create a `galleryImages` record until an explicit review action. This keeps exact repeats outside the gallery while allowing every other file in the same batch to continue independently.

An isolated mixed-batch QA run verified two normal uploads completed while one exact and one visual duplicate both entered the durable queue. Refreshing Studio preserved the queue, and the rendered desktop inbox displayed the correct count and clear “Review next” entry point. Resolving the exact candidate with **Keep old image** automatically advanced to the visual candidate and updated the remaining count from two to one. The final visual candidate was resolved through the rendered **Upload as new** action; it became unorganised and received only an editable Gemini suggestion, with no automatic album move.

The controlled QA harness also verified **Keep old**, **Upload as new**, and **Replace with new**. Replacement retained the temporary matched image’s album membership and now runs a pending Gemini suggestion without creating or moving albums. It verified **Apply to Similar · Upload new** resolves only pending visual candidates mapped to the same existing image, while **Apply to All** is deliberately limited to the conservative Keep old decision. The exact-repeat policy rejects bulk or individual Upload as new/Replace actions.

Desktop and 390 × 844 mobile captures confirmed the premium Needs Review card remains legible, with the mobile review entry point expanded to a full-width touch target. The desktop dialog showed side-by-side stored candidate and existing image panels, exact-match guarding, queue progress, and scoped bulk controls. Final cleanup removed all `qa-duplicate-queue-*` and `qa-review-visual-*` records: zero pending reviews, zero temporary candidates, zero temporary gallery records, and 24 retained user gallery images remain.
