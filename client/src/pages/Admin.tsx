...
export default function Admin() {
  ...
  const selectedForAnalysis = selected.filter(imageId => {
    const image = libraryImages.find(item => item.recordId === imageId);
    return image?.aiStatus !== "ready" && image?.aiStatus !== "approved" && image?.aiStatus !== "analyzing" && image?.aiStatus !== "queued";
  });
  const analyzeSelected = async () => {
    if (!aiSettings.enabled || !selectedForAnalysis.length) return;
    try {
      const result = await analyzeUnorganisedJewelleryBatch.mutateAsync({ imageIds: selectedForAnalysis.slice(0, aiSettings.batchSize) });
      addActivity("Gemini batch reviewed", `${result.results.filter(item => item.status === "ready").length} unorganised upload${selectedForAnalysis.length === 1 ? "" : "s"} received suggestions`);
      setSelected([]);
      await utils.gallery.publicDashboard.invalidate();
    } catch {
      addActivity("Gemini batch needs attention", "The selected unorganised uploads could not be analysed. Check the model and secure key in Presentation settings.");
    }
  };
...
function AiReview({ images, albums, onApprove, onDismiss, onApproveAll, saving, batchSaving }: { images: PersistedGalleryImage[]; albums: PersistedAlbum[]; onApprove: (imageId: number, name: string, description: string, assignAlbum: boolean) => void; onDismiss: (imageId: number) => void; onApproveAll: (imageIds: number[]) => void; saving: boolean; batchSaving: boolean }) {
  ...
  return <section className="ai-review-panel"><div className="ai-review-heading"><div><p className="eyebrow">GEMINI BATCH REVIEW</p><h2>Review before organizing</h2></div><div className="ai-review-batch-action"><span>{suggestions.length}</span>{suggestions.length > 1 && <button className="secondary-button" disabled={batchSaving} onClick={() => onApproveAll(suggestions.map(image => image.recordId))}>{batchSaving ? "Applying…" : `Apply all ${suggestions.length}`}</button>}</div></div><p className="ai-review-intro">Names, descriptions, and album ideas are suggestions. Edit an item or apply the reviewed batch when it is right.</p><div className="ai-review-grid">{suggestions.map(image => <AiReviewCard key={image.recordId} image={image} albums={albums} onApprove={onApprove} onDismiss={onDismiss} saving={saving || batchSaving} />)}</div></section>;
}
...
function PresentationSettings({ settings, setSettings, aiSettings, personalKeyConfigured, onAiSettingsChange, savingAiSettings }: { settings: { autoplay: boolean; loop: boolean; interval: string; transition: string; showCaptions: boolean }; setSettings: React.Dispatch<React.SetStateAction<{ autoplay: boolean; loop: boolean; interval: string; transition: string; showCaptions: boolean }>>; aiSettings: AiSettingsValue; personalKeyConfigured: boolean; onAiSettingsChange: (changes: Partial<AiSettingsValue>) => void; savingAiSettings: boolean }) {
  ...
  return <section className="admin-full-page settings-page"><div className="admin-page-heading"><div><p className="eyebrow">PRESENTATION & AI</p><h1>Simple presentation and jewellery assistance.</h1><p>Uploads stay unorganised first. Select a batch in Library whenever you want Gemini to create reviewable suggestions.</p></div></div><div className="settings-grid"><section><h2>Playback</h2>{toggle("autoplay", "Autoplay")}{toggle("loop", "Loop continuously")}<label>Interval<select value={settings.interval} onChange={event => setSettings(current => ({ ...current, interval: event.target.value }))}>{[2,3,5,8,10,15,30].map(value => <option value={value} key={value}>{value} seconds</option>)}</select></label></section><section><h2>Appearance</h2>{toggle("showCaptions", "Show captions")}<label>Transition<select value={settings.transition} onChange={event => setSettings(current => ({ ...current, transition: event.target.value }))}><option value="crossfade">Crossfade</option><option value="fade">Fade</option><option value="slide">Slide</option><option value="instant">Instant</option></select></label></section><section className="ai-settings-card"><div className="ai-settings-title"><Sparkles size={17} /><h2>Gemini batch review</h2></div><p>Gemini only analyses selected unorganised uploads. It never creates or moves an album until you apply a review.</p><label className="settings-toggle"><span>Enable Gemini batch review</span><input type="checkbox" checked={aiSettings.enabled} disabled={savingAiSettings} onChange={event => onAiSettingsChange({ enabled: event.target.checked, autoAnalyzeNew: false })} /></label><label>AI provider<select value={aiSettings.provider} disabled={savingAiSettings} onChange={event => { const provider = event.target.value as AiSettingsValue["provider"]; onAiSettingsChange({ provider, model: provider === "personal" ? "gemini-3.1-flash-lite" : "gemini-3-flash-preview" }); }}><option value="builtin">Movement Gemini</option><option value="personal">My Gemini API key</option></select></label><label>AI model<select value={aiSettings.model} disabled={savingAiSettings || !aiSettings.enabled} onChange={event => onAiSettingsChange({ model: event.target.value as AiSettingsValue["model"] })}>{models.map(model => <option value={model.value} key={model.value}>{model.label}</option>)}</select></label><label>Images per batch<select value={aiSettings.batchSize} disabled={savingAiSettings || !aiSettings.enabled} onChange={event => onAiSettingsChange({ batchSize: Number(event.target.value) })}>{[1, 2, 4, 6, 8, 12].map(value => <option value={value} key={value}>{value} images</option>)}</select></label>{personalProvider && <div className="personal-key-note"><b>{personalKeyConfigured ? "Personal key connected securely" : "Personal key needed"}</b><span>{personalKeyConfigured ? "Your Gemini API key stays server-side and is never shown in Movement." : "Add or replace GEMINI_API_KEY securely in Project Settings → Secrets; it will never be shown in Movement."}</span></div>}<small>Use the built-in provider with Movement’s managed Gemini models, or use your saved Google Gemini API key with Gemini 3.1 / 3.5 Flash-Lite.</small></section></div></section>;
}
