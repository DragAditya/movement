...
export default function Admin() {
  ...
  return <div className="admin-mode-shell album-admin-shell"><AppHeader mode="admin" /><div className="admin-app"><aside className="admin-sidebar"><nav className="admin-nav">{nav.map(item => <button key={item.key} className={tab === item.key ? "is-active" : ""} onClick={() => setTab(item.key)}><item.icon size={18} /><span>{item.label}</span></button>)}</nav><div className="admin-account"><div className="account-avatar">A</div><div><b>Admin</b><span>Album workspace</span></div></div></aside><main className="admin-content">
    ...
    {tab === "presentation" && <PresentationSettings settings={settings} setSettings={setSettings} aiSettings={aiSettings} onAiSettingsChange={changes => void updateAiSettings.mutateAsync(changes).then(() => utils.gallery.publicDashboard.invalidate())} savingAiSettings={updateAiSettings.isPending} />}
...
type AiSettingsValue = { enabled: boolean; autoAnalyzeNew: boolean; model: "gemini-3-flash-preview" | "gemini-3.1-pro-preview" };

function AiReview({ images, albums, onApprove, onDismiss, saving }: { images: PersistedGalleryImage[]; albums: PersistedAlbum[]; onApprove: (imageId: number, name: string, description: string, assignAlbum: boolean) => void; onDismiss: (imageId: number) => void; saving: boolean }) {
  const suggestions = images.filter(image => image.aiStatus === "ready");
  if (!suggestions.length) return null;
  return <section className="ai-review-panel"><div className="ai-review-heading"><div><p className="eyebrow">AI ASSISTANCE</p><h2>Ready to organize</h2></div><span>{suggestions.length}</span></div><div className="ai-review-grid">{suggestions.map(image => <AiReviewCard key={image.recordId} image={image} albums={albums} onApprove={onApprove} onDismiss={onDismiss} saving={saving} />)}</div></section>;
}
...
function PresentationSettings({ settings, setSettings, aiSettings, onAiSettingsChange, savingAiSettings }: { settings: { autoplay: boolean; loop: boolean; interval: string; transition: string; showCaptions: boolean }; setSettings: React.Dispatch<React.SetStateAction<{ autoplay: boolean; loop: boolean; interval: string; transition: string; showCaptions: boolean }>>; aiSettings: AiSettingsValue; onAiSettingsChange: (changes: Partial<AiSettingsValue>) => void; savingAiSettings: boolean }) {
  ...
  return <section className="admin-full-page settings-page"><div className="admin-page-heading"><div><p className="eyebrow">PRESENTATION & AI</p><h1>Simple presentation and jewellery assistance.</h1><p>AI is optional. When enabled, only new uploads receive a short name, a short description, and one album suggestion.</p></div></div><div className="settings-grid"><section><h2>Playback</h2>{toggle("autoplay", "Autoplay")}{toggle("loop", "Loop continuously")}<label>Interval<select value={settings.interval} onChange={event => setSettings(current => ({ ...current, interval: event.target.value }))}>{[2,3,5,8,10,15,30].map(value => <option value={value} key={value}>{value} seconds</option>)}</select></label></section><section><h2>Appearance</h2>{toggle("showCaptions", "Show captions")}<label>Transition<select value={settings.transition} onChange={event => setSettings(current => ({ ...current, transition: event.target.value }))}><option value="crossfade">Crossfade</option><option value="fade">Fade</option><option value="slide">Slide</option><option value="instant">Instant</option></select></label></section><section className="ai-settings-card"><div className="ai-settings-title"><Sparkles size={17} /><h2>AI Assistance</h2></div><p>For new jewellery uploads only. Nothing moves until you approve it.</p><label className="settings-toggle"><span>Enable AI assistance</span><input type="checkbox" checked={aiSettings.enabled} disabled={savingAiSettings} onChange={event => onAiSettingsChange({ enabled: event.target.checked })} /></label><label>AI model<select value={aiSettings.model} disabled={savingAiSettings || !aiSettings.enabled} onChange={event => onAiSettingsChange({ model: event.target.value as AiSettingsValue["model"] })}><option value="gemini-3-flash-preview">Gemini Flash — Default</option><option value="gemini-3.1-pro-preview">Gemini 3.1 Pro — Higher quality</option></select></label><small>Creates a short English name, short description, and one album suggestion.</small></section></div></section>;
}
