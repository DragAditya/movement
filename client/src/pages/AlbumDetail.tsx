...
import { AlbumDetailSkeleton } from "@/components/GallerySkeletons";
...
export default function AlbumDetail() {
  ...
  const permanentAlbum = systemAlbum?.slug === slug ? systemAlbum : undefined;
  const customAlbum = customAlbums.find(item => item.slug === slug);
  const activeAlbum = permanentAlbum ?? customAlbum;
  const images = smart ? smartImages(smart.id) : activeAlbum ? albumImages(activeAlbum.id) : [];
  const title = smart?.name ?? activeAlbum?.name;
  const description = smart?.description ?? activeAlbum?.description ?? "";
  const mode = activeAlbum?.mode ?? "immersive";
  ...
  if (isLoading) return <div className="public-shell"><AppHeader mode="gallery" active="albums" /><main className="public-page album-detail-page"><AlbumDetailSkeleton /></main></div>;
